...
import type { TokenLeaf } from "~/lib/brand-tokens"
...
export function TokenInspector({
  ...
  return (
    <div className="grid gap-4 p-4">
        ...
        <>
          ...
          {format === "oklch" && (
            <p className="text-xs text-muted-foreground">
              Saved as {toBrandValue(parsed, "oklch")}. brand-settings.json
              holds rgb, rgba and 6-digit hex. Refs stay refs.
            </p>
          )}
      ...
      {token.kind === "ref" || token.kind === "paint" ? (
        <p className="text-xs text-muted-foreground">
          Palette ref. Saved as written — not converted to rgb.
        </p>
      ) : null}
