...
async function compileState(slug: string, deck: Deck) {
  ...
  if (deck.brand) {
    sources.push(path.join(REPO_ROOT, "brands", deck.brand, "brand-settings.json"))
    sources.push(path.join(REPO_ROOT, "brands", deck.brand, "brand.css"))
  }
...
export type Preset = {
  ...
  relativePath: string
  /**
   * A preset with a `<slide>` can be copied into a deck. Anything else is a
   * fragment and the compiler would reject it.
   */
  kind: "slide" | "fragment"
}
...
/**
 * Preset HTML is rooted at its own folder depth. A deck slide always lives at
 * `decks/{slug}/`, two levels down, so brand slides (`presets/brands/{slug}/`)
 * need `../../../` rewritten to `../../`.
 */
export function rewritePresetPaths(html: string, presetRelativePath: string): string {
  const from = "../".repeat(presetRelativePath.split(/[/\\]/).length)
  const to = "../../"
  if (from === to) return html
  return html
    .replaceAll(`${from}design-system/`, `${to}design-system/`)
    .replaceAll(`${from}assets/`, `${to}assets/`)
}

export async function listPresets(): Promise<Preset[]> {
  const files = await htmlFiles(PRESETS_DIR)
  const presets: Preset[] = []
  for (const full of files.sort()) {
    const relativePath = path.relative(PRESETS_DIR, full)
...
export async function addSlideFromPreset(
  ...
  await fs.writeFile(
    path.join(deckDir(slug), file),
    rewritePresetPaths(html, presetRelativePath),
    "utf8"
  )
  return file
}
