 ...
 *
 * 1. **What can be written as a colour literal.** `parseColor()` in
 *    `scripts/validate-brand.js` accepts `rgb()`, `rgba()` and 6-digit
 *    `#rrggbb` only. Palette refs are not colours. oklch is an *editing*
 *    space here — you can read and type it, and `toBrandValue` converts it
 *    back before anything reaches `brand-settings.json`.
...
/** Formats a colour literal may use. Palette refs are separate. */
