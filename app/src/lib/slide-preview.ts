  ...
  if (brand) {
    try {
      const settings = JSON.parse(
        await fs.readFile(
          path.join(REPO_ROOT, "brands", brand, "brand-settings.json"),
          "utf8"
        )
      ) as { foundations?: { basic?: { logo?: string } } }
      const logo = settings.foundations?.basic?.logo || `${brand}-logo.svg`
      const inverted = logo.replace(/\.svg$/i, "-inverted.svg")
      logoAttrs = `data-logo="../../brands/${brand}/${logo}" data-logo-inverted="../../brands/${brand}/${inverted}"`
    } catch {
      logoAttrs = ""
    }
  }
