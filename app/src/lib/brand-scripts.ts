...
/** Regenerates `brand.css` from `brand-settings.json`. Always follows a write. */
export async function generateBrandCss(absoluteBrandDir: string) {
  ...
  if (failed) {
    throw new Error(
      `brand-settings.json was written but brand.css could not be regenerated: ${stderr.trim()}`
    )
  }
}
