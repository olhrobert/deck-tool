...
First real surface on the shell. A brand is a token override — `brand-settings.json` is the file, `brand.css` is generated, and the app does not keep a second copy of the tokens. Logos are a baked pair (default + inverted). See `../../../docs/brands.md` for the model.
...
| View | `/brands/[slug]` | Tokens, logo, validation result, decks using it |
...
**Archive is a field, not a move.** `"archived": true` on `foundations.basic` in `brand-settings.json`. The CSS generator ignores it: an extra key is inert. Moving the directory would break every `slides.json` that references the slug.
