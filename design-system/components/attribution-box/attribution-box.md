...
not_when:
  - Speaker name/role on a founder narrative (plain text)
  - The deck/client logo (that is `data-logo`, not this component)
---

# Attribution box

Brand-agnostic Gratia credit. Do not tokenize it in `brand-settings.json`. Leave `<img data-slot="logo">` pointing at `../../assets/logos/gratia-logo.svg`. Only the credit text is editable. The vertical separator uses `--divider-color`.
