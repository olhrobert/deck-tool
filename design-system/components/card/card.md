...
Use `<card padding="md">` with `<card-pretitle>` (`tone="subtle" context="surface"`), `<card-title>`, optional `<text data-slot="text" size="350" tone="base" context="surface">`, and optional `<card-meta>`. Omit `gap` so `sm` applies. Omit `layout` so `components.card.defaultLayout` applies.
