...
function main() {
	...
	if (!template.foundations) template.foundations = {};
	if (!template.foundations.basic) template.foundations.basic = {};
	template.foundations.basic.name = name || titleCaseSlug(slug);
	const logoFile = `${slug}-logo.svg`;
	const logoInvertedFile = `${slug}-logo-inverted.svg`;
	template.foundations.basic.logo = logoFile;
