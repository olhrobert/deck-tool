...
function usage() {
	...
	console.error("Example: node scripts/compile-deck.js decks/test");
	process.exit(1);
}
...
function logoAttrs(brandName) {
	if (!brandName) return "";
	const settingsPath = path.join(BRANDS_DIR, brandName, BRAND_FILENAME);
	const settings = JSON.parse(fs.readFileSync(settingsPath, "utf8"));
	const logo =
		(settings.foundations &&
			settings.foundations.basic &&
			settings.foundations.basic.logo) ||
		`${brandName}-logo.svg`;
	const inverted = logo.replace(/\.svg$/i, "-inverted.svg");
	return [
		`data-logo="../../brands/${brandName}/${logo}"`,
		`data-logo-inverted="../../brands/${brandName}/${inverted}"`,
	].join(" ");
}
...
function compileDeck(deckDir, { sync = true } = {}) {
	...
	const output = template
		...
		.replace("{{logoAttrs}}", logoAttrs(manifest.brand))
