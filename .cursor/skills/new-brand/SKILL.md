...
6. Figma (`figma/library.json` `fileKey`), `use_figma` with `skillNames`: `figma-use,figma-generate-library`:
    ...
    - Write primitive color values for that mode from `brand-settings.json` (map via `primitive/color/*` dsb keys — role names: `slide-foreground`, `cover-background`, …). HTML canvas is `slide.canvas.background.light|dark`; until the Figma Color Light/Dark pass, still write `cover-background` from the cover color-theme’s canvas pair.
